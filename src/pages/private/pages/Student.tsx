...
const Student = () => {
  ...
  const { students, searchStudents } = useCrudStore();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row items-center justify-between">
          ...
          <div className="md:mt-0 flex items-center space-x-4">
            ...
            <button
              ...
              className="flex items-center space-x-1 hover:bg-blue-600/80 bg-blue-500 px-4 py-1.5 rounded-lg transition"
            >
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 py-5">
            <div className="bg-indigo-50 rounded-xl p-5 border border-indigo-100">
              ...
              <p className="text-2xl font-bold text-gray-900 mt-2">{totelLenInfo?.totalStudents || 0}</p>
              <p className="text-xs text-indigo-600 mt-1">
                {totelLenInfo?.studentRateChange} rate of {totelLenInfo?.totalStudents}
              </p>
            </div>
            ...
            <div className="bg-emerald-50 rounded-xl p-5 border border-emerald-100">
              ...
              <p className="text-2xl font-bold text-gray-900 mt-2">{students?.filter(s => s.isActiveClass)?.length || 0}</p>
              <p className="text-xs text-emerald-600 mt-1">
                <span className="font-semibold">{students?.filter(s => !s.isActiveClass)?.length || 0}</span> Inactive
              </p>
            </div>
            ...
            <div className="bg-amber-50 rounded-xl p-5 border border-amber-100">
              ...
              <p className="text-2xl font-bold text-gray-900 mt-2">{students?.filter(s => s.feePaid)?.length || 0}</p>
              <p className="text-xs text-amber-600 mt-1">Review required</p>
            </div>
          </div>

          {/* Students Table */}
          <GetStudents searchTerm={searchTerm} data={students} />
        </div>
      </main>
    </div>
  );
